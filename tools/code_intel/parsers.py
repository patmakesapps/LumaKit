...
def parse_file(file_path: str, source: str | None = None) -> tuple[list[Symbol], list[Reference]]:
    ...
    extract_params = _extract_params_python if is_python else _extract_params_js

    # Module prefix for qualified names
    module = file_path.replace("\\", "/").replace("/", ".").removesuffix(".py").removesuffix(".js").removesuffix(".ts").removesuffix(".tsx").removesuffix(".jsx")
