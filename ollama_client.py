...
class OllamaClient:
    ...
    def _post_with_fallback(self, model, messages, tools, stream):
        """Try primary model, fall back on connection failure."""
        try:
            result = self._post(model, messages, tools, stream)
            ...
            # Try fallback
            try:
                result = self._post(self.fallback_model, messages, tools, stream)
    ...
    def chat(self, model, messages, tools=None, stream=False, deadline=None):
        ...
        if deadline is None:
            return self._post_with_fallback(model, messages, tools, stream)

        with ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(
                self._post_with_fallback, model, messages, tools, stream
            )
            try:
